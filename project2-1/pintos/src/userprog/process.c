...
start_process (void *file_name_)//child excute this function not parent
{
  ...
  bool success;
 // printf("START_PROCESS : TID : %i",thread_current()->tid);
...
process_wait (tid_t child_tid UNUSED) //child_tid check
{
  ...
  sema_init(&Target_child->wait_child,0);
  sema_down(&Target_child->wait_child);
  return Target_child->exit_state;
 // struct thread* Target_child=ret_thread(child_tid);
 /*  if(Target_child->status==THREAD_DYING||Target_child->status==THREAD_DIED_EXIT)//already terminated before call wait
  {
    return -1;
  }
  else if(Target_child->parent_ptr!=thread_current())
  return -1;
  else{
 while(1){
  ...
  else if(Target_child->tid!=child_tid)
  {
    return 1;
  }if(ret_thread(child_tid)->status==THREAD_DIED_EXIT||ret_thread(child_tid)->status==THREAD_DYING){
return 1;}} 
}*/ 
}
